...
from Padim.visualization import heatmap_image
...
if __name__ == "__main__":
    ...
    def run():
        ...
        def commuicate_with_plc():
            while isGrabbing and is_padim_model_inited and plc_s7.get_connected():
                ...
                if check_equal_value(get_value, 1): # may bug this line
                    t = time.time()
                    ...
                    # image_scores: tensor
                    # score_maps: tensor with shape 1xHxW
                    
                    # draw output
                    heatmap = heatmap_image(img, score_maps[0],min_v=0, max_v=THRESH, alpha=0.5) # HxWxC
                    heatmap = cv2.resize(heatmap, (500,400))
                    qImg = QtGui.QImage(heatmap, 
                                        heatmap.shape[1], 
                                        heatmap.shape[0], 
                                        heatmap.shape[1] * 3, 
    ...
    def init_plc():
        if not plc_s7.get_connected():
            ...
            print("Plc is alraedy connected!")
